...
{
public:
	PtMass(Eigen::Vector3f pos, float mass)
	{
		m_pos, m_new_pos, m_prev_pos = pos;
		m_vel, m_new_vel, m_prev_vel = Eigen::Vector3f(0.0f, 0.0f, 0.0f);
		m_mass = mass;
	}
	...
	void step_vel(Eigen::Vector3f pos, float dt)
	{
...
{
public:
	Rope(float _k, float _c, Eigen::Vector3f start_pos, Eigen::Vector3f end_pos, int _num_pts,
		...
		float _drag_coef)
	{
		...
		k = _k;
		c = _c;
		pts = new Eigen::Vector3f[_num_pts];
		num_pts = _num_pts;
		...
		GRAVITY = Eigen::Vector3f(0.0f, -gravity, 0.0f);
		reset();
	};
